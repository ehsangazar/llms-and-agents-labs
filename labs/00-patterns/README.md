...
```bash
npm run lab labs/00-patterns/hook/naive.ts
```
...
```bash
npm run lab labs/00-patterns/hook/real-traffic.ts
```
...
```bash
npm run lab labs/00-patterns/hook/hardened.ts
```
...
```bash
npm run lab labs/00-patterns/patterns/01-prompt-chaining.ts
```
...
```bash
npm run lab labs/00-patterns/patterns/02-routing.ts
```
...
```bash
npm run lab labs/00-patterns/patterns/03-parallelization.ts
```
...
```bash
npm run lab labs/00-patterns/patterns/04-orchestrator-workers.ts
```
...
```bash
npm run lab labs/00-patterns/patterns/05-evaluator-optimizer.ts
```
...
Run both, back to back. The comparison is the point.

### `as-workflow.ts` — fixed sequence

```bash
npm run lab labs/00-patterns/workflow-vs-agent/as-workflow.ts
```

**Watch for:** the call count. Predictable calls, predictable cost.

### `as-agent.ts` — a loop that decides when it is done

```bash
npm run lab labs/00-patterns/workflow-vs-agent/as-agent.ts
```
...
Session 1's hands-on lab is [Lab 1 · Build a Workflow Router](../01-workflow-router).
Start there with `npm test`, which needs no API key.
